...
const PHI_POOL = [0.3, 0.5, 0.7, 0.85] as const; // фиксированный набор (можешь менять)
...
export default function ForecastGamePage() {
  ...
  const [revealUpto, setRevealUpto] = useState(-1); // -1 = пока факты не показываем
  ...
  const [phi, setPhi] = useState<Phi>(PHI_POOL[0]);
  ...
  const handleConfirmStep = useCallback(() => {
    ...
    // 2) СРАЗУ показываем факт для этого шага
    setRevealUpto(activeStepIndex);

    const nextIndex = activeStepIndex + 1;

    // 3) если ещё есть шаги — переходим дальше
    if (nextIndex < NUM_STEPS) {
      ...
      // (опционально) чуть сдвинуть вправо
      setScrollPosition(100);
      return;
    }

    // 4) если это последний шаг — завершаем раунд и считаем score
    ...
    for (let i = 0; i < NUM_STEPS; i++) {
      const predVal = preds[i]?.final ?? (i === activeStepIndex ? activePred.final : null);
    ...
    setStatus({
      ...
      message: `Раунд завершён! Очки: ${score}/100 (MSE: ${mse.toFixed(2)}).`,
    });
  ...
  const handleSendResults = useCallback(async () => {
    try {
      ...
      setStatus({
        type: "success",
        message: `Результаты сохранены! Среднее: ${avgScore}/100. Итого: ${totalScore}/${TOTAL_ROUNDS * 100}.`,
      });
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      if (showTruth) {
        for (let s = 0; s <= revealUpto; s++) {
          const idx = STEP_DEFS[s].offset - 1;
      ...
      if (showTruth) {
        ...
        ctx.moveTo(toX(HISTORY_LEN - 1), toY(history[history.length - 1]));

        // Draw truth only up to revealUpto
        for (let s = 0; s <= revealUpto; s++) {
          const truthIdx = STEP_DEFS[s].offset - 1;
          if (truthIdx < truth.length) {
            ctx.lineTo(stepX(s), toY(truth[truthIdx]));
          }
        }
        ...
        // Truth dots only up to revealUpto
        for (let s = 0; s <= revealUpto; s++) {
          const truthIdx = STEP_DEFS[s].offset - 1;
          if (truthIdx < truth.length) {
            const px = stepX(s);
    ...
    draw();
  }, [rounds, roundSubmitted, activeStepIndex, revealUpto]);
    ...
    <div className="gradient-mesh min-h-screen min-h-[100dvh] flex flex-col">
      ...
      <div className="px-3 pb-2">
        <button
          ...
          className="w-full glass rounded-2xl px-4 py-3 text-sm font-mono"
        >
          Открыть рейтинг
        </button>
      </div>
